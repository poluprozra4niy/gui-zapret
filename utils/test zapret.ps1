...
function Read-TestType {
    if ($TestTypeParam) {
        Write-Host "Test Type selected via argument: $TestTypeParam" -ForegroundColor Cyan
        return $TestTypeParam
    }
...
function Read-ModeSelection {
    if ($RunModeParam) {
        Write-Host "Run Mode selected via argument: $RunModeParam" -ForegroundColor Cyan
        return $RunModeParam
    }
...
function Read-ConfigSelection {
    ...
    if ($SelectedFilesParam) {
        Write-Host "Selected files via argument: $SelectedFilesParam" -ForegroundColor Cyan
        $names = $SelectedFilesParam -split "," | ForEach-Object { $_.Trim() }
        $valid = @()
        foreach ($name in $names) {
            ...
            if ($found) {
                ...
                Write-Host "Warning: Config '$name' not found." -ForegroundColor Yellow
            }
        }
        if ($valid.Count -gt 0) {
            return $valid
        }
        Write-Host "No valid configs found in argument. Falling back to interactive selection." -ForegroundColor Red
    }
